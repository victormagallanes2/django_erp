...
console.log("🔴 SCRIPT DE FACTURAS DE COMPRA CARGADO - CON PRECIO Y STOCK");
...
function recalculateInvoiceTotals() {
    ...
    // ✅ También actualizar los campos de display si existen (para órdenes de compra)
    var subtotalDisplay = document.getElementById('id_subtotal_display');
    var taxDisplay = document.getElementById('id_tax_display');
    var totalDisplay = document.getElementById('id_total_display');
    ...
    if (taxDisplay) {
        taxDisplay.value = formatNumber(tax);
        taxDisplay.textContent = formatNumber(tax);
    }
    if (totalDisplay) {
    ...
    // ✅ Actualizar campos en Bs. si existen
...
function setupRow(row) {
    ...
    // ✅ Cuando cambia el precio (si no es readonly)
    if (priceInput) {
...
function setupAllRows() {
    console.log("🔴 Configurando líneas de factura de compra...");
    var rows = document.querySelectorAll('tr.form-row');
    rows.forEach(function(row, index) {
        // ✅ Configurar solo filas que tienen select de producto
        if (row.querySelector('select[id$="-product"]')) {
            if (!row._hasSetup) {
                row._hasSetup = true;
                setupRow(row);
            }
...
document.addEventListener('click', function(e) {
    ...
    if (addButton) {
        ...
        setTimeout(function() {
            var rows = document.querySelectorAll('tr.form-row');
            rows.forEach(function(row) {
                if (row.querySelector('select[id$="-product"]') && !row._hasSetup) {
                    row._hasSetup = true;
                    setupRow(row);
                }
            });
...
// ✅ Inicializar
function initialize() {
    console.log("🔴 INICIALIZANDO FACTURAS DE COMPRA...");
    setupAllRows();
